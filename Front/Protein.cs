...
namespace Front
{
    public partial class Protein : Form
    {
        ...
        public Protein()
        {
            ...
            this.Shown += (s, e) => LoadProteinItems();
        }
        ...
        private void LoadProteinItems()
        {
            dgvProtein.AutoGenerateColumns = true;
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = "SELECT foodID, foodName, Unit, Quantity, openingDate, shelfLife, Status FROM Protein";
                SqlDataAdapter adapt = new SqlDataAdapter(query, conn);
                DataTable dt = new DataTable();
                adapt.Fill(dt);
                dgvProtein.DataSource = dt;
            }
            if (dgvProtein.Columns.Contains("foodName"))
                dgvProtein.Columns["foodName"].HeaderText = "Food Name";
            if (dgvProtein.Columns.Contains("Unit"))
                dgvProtein.Columns["Unit"].HeaderText = "Unit";
            if (dgvProtein.Columns.Contains("Quantity"))
                dgvProtein.Columns["Quantity"].HeaderText = "Quantity";
            if (dgvProtein.Columns.Contains("openingDate"))
                dgvProtein.Columns["openingDate"].HeaderText = "Opening Date";
            if (dgvProtein.Columns.Contains("shelfLife"))
                dgvProtein.Columns["shelfLife"].HeaderText = "Shelf Life";
            if (dgvProtein.Columns.Contains("Status"))
                dgvProtein.Columns["Status"].HeaderText = "Status";

            dgvProtein.Columns["foodID"].Visible = false;
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                string query = "SELECT foodID, foodName, Unit, Quantity, openingDate, shelfLife, Status " + "FROM Protein WHERE foodName LIKE @search";
                SqlDataAdapter adapt = new SqlDataAdapter(query, conn);
        ...
        private void btnRemove_Click_1(object sender, EventArgs e)
        {
            ...
            if (dgvProtein.SelectedRows.Count > 0)
                sr = dgvProtein.SelectedRows[0];
            else if (dgvProtein.CurrentRow != null)
                sr = dgvProtein.CurrentRow;
            else
            {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction transac = conn.BeginTransaction())
                {
                    try
                    {
                        string deleteQuery = "DELETE FROM Protein WHERE foodID = @id";
                        ...
                        using (SqlCommand cmd1 = new SqlCommand(insertAudit, conn, transac))
                        {
                            ...
                            cmd1.Parameters.AddWithValue("@actionDone", $"Removed '{foodName}' from Protein Table");
        ...
        private void btnInsert_Click_2(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    string query = @"INSERT INTO Protein (foodName, Unit, Quantity, openingDate, shelfLife) VALUES (@foodName, @Unit, @Quantity, @openingDate, @shelfLife)";
